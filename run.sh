...
if [ -n "{{MODEL}}" ] && [ -n "{{DATASET_PREFIX}}" ]  && [ -n "{{OUTPUT_DIR}}" ] && [ -n "{{TPU}}" ] && [ -n "{{STEP}}" ]; then
    ...
    export FILE_PATTERN="/media/tavandai/MAIN_WORKING_UBUNTU/Startup/image_tagging/datasets/imaterialist-fashion-2021-fgvc8/kaggle-imaterialist2020-model/data_processed/tfrecord/test-*"
